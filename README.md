...
我们的做法是提出 `Crew Type` 的概念（便于按模板套用权限和技能组合），并尽量少的塞入预置 crew，在保证有效基础支撑的前提下，把更多自定义空间留给你，因为我们知道想搞钱，就要有特色，你的业务不同，你需要的 crew 也不同！

在 OFB，Crew 分为两大类：`对内`和`对外`.
...
- `对外crew` 负责服务外部客户，是帮你“搞钱”的。
  ...
  - 我们也会不断推出官方模板，并且后面会引入 marketplace（addon 市场）机制，提供丰富的 crew 模板。
  - 目前我们已经随代码仓提供了第一个对外 crew 模板：`sales customer service` （销售导向客服）

有关 OFB 的“多 crew 机制”设计，详见[CREW TYPE DESIGN](crews/DESIGN.md)
...
目前 ofb 已经默认配置 `it-engineer` 为所有对内 crew 可 spawn，这令我们可以不必为一个任务分别找不同的 crew以及在任务执行过程中遇到问题，crew 也会自动唤起 it-engineer 进行协查：
...
我们会陆续放出“开箱即用”的 crew 模板，你可以通过main agent 或者 HRBP 招募启用。

所有的模板都是在实战中打磨出来的。

目前已经提供了第一个 crew 模板 —— **销售型客服**

它不是单纯的回答客户咨询的客服，它以促进成交为目的，会在咨询答疑过程中以用户无感的巧妙话术：
- 1、调研用户的来源（便于投流/SEO 改进）；
- 2、努力问询客户的应用场景，寻找与我方的价值同频点；
- 3、通过配置的各种手段促进成交（发送 demo 视频、邀请体验、推荐合适价位的产品/服务）；
- 4、根据对话进展，自动选择合适时机发起收款；
- 5、内置 sqlite 和 hook 工具，记录客户状态等数据（IT Engineer 可以执行每日导出数据、同步 feishu）

目前 wiseflow Team 的在线客户“小掌柜”就是使用的这套模板，感兴趣可以微信扫码：
...
☀️ 想要同款销售型客服？部署 OFB 后，通过 hrbp 启用，之后可以通过 hrbp “调教” 它适配你自己的业务：

<img width="960" src="assets/hr-skill-creator.png" />
